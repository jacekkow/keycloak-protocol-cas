...
public abstract class BaseCasLoginIT {
  ...
  void beforeEach() {
    ...
    try (Keycloak keycloakClient = keycloakClient()) {
  ...
  void assertTicketContent() {
    goToRealmAndCreateNewClient(page, keycloakClient(), getAuthServerUrl(), REALM_NAME, CAS_CLIENT_1, getAdminUser(), getAdminPassword(), SERVICE_URL_1);

    // Trigger CAS authorization and login fully in the browser
    String casLoginUrl = getAuthServerUrl() + "/realms/" + REALM_NAME + "/protocol/cas/login?service=" + urlEncode(SERVICE_URL_1);
    page.close();
    page = playwrightDefaults.setUp();
    ...
    Response validate =
        RestAssured.given()
            ...
            .get(String.format("%s/realms/%s/protocol/cas/serviceValidate", getAuthServerUrl(), REALM_NAME))
            .then()
            .statusCode(200)
            .extract()
            .response();
