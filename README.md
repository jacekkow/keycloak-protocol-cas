# keycloak-protocol-cas
  ...
  * Long-Term Tickets - Remember-Me [CAS 3.0 - optional]
  ...
  Quarkus is the default distribution method of Keycloak 17.0.0 and newer. For legacy installations using WildFly,
  please refer to the [old README](https://github.com/jacekkow/keycloak-protocol-cas/blob/16.1.1/README.md).

  1. Download the latest release compatible with your Keycloak version from the [releases
  page](https://github.com/jacekkow/keycloak-protocol-cas/releases).
  2. Put the downloaded JAR file into the `providers/` directory inside Keycloak installation folder. If necessary,
  adjust the permissions/ownership so that the user Keycloak runs as is able to read this file.
  ...
  **Redirect URI Configuration:** Avoid using wildcard (`*`) redirect URIs for CAS clients. When wildcard redirect URIs
  are configured, any domain can receive service tickets, which enables open redirect and potential service ticket theft
   if combined with the CAS `gateway` parameter (see
  [#185](https://github.com/jacekkow/keycloak-protocol-cas/issues/185)). Always configure specific, exact redirect URIs
  for each CAS-enabled application.
